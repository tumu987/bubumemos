...
const MemoView: React.FC<MemoViewProps> = (props: MemoViewProps) => {
  ...
  const [pdfPreviewState, setPdfPreviewState] = useState<{ open: boolean; items: PreviewMediaItem[]; index: number }>({
    open: false,
    items: [],
    index: 0,
  });
  const openPdfPreview = useCallback((items: PreviewMediaItem[], index = 0) => {
  ...
  const article = (
    ...
    >
      ...
      <PdfPreviewDialog
        ...
        onOpenChange={(open) => setPdfPreviewState((prev) => ({ ...prev, open }))}
