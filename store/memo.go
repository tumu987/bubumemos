...
func (s *Store) DeleteMemo(ctx context.Context, delete *DeleteMemo) error {
	return s.driver.RunInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err := s.driver.DeleteMemoRelation(txCtx, &DeleteMemoRelation{MemoID: &delete.ID}); err != nil {
			return err
		}
		if err := s.driver.DeleteMemoRelation(txCtx, &DeleteMemoRelation{RelatedMemoID: &delete.ID}); err != nil {
			return err
		}
		// Clean up attachments linked to this memo.
		attachments, err := s.ListAttachments(txCtx, &FindAttachment{MemoID: &delete.ID})
		if err != nil {
			return err
		}
		for _, attachment := range attachments {
			if err := s.DeleteAttachment(txCtx, &DeleteAttachment{ID: attachment.ID}); err != nil {
				return err
			}
		}
		return s.driver.DeleteMemo(txCtx, delete)
	})
}
